...
def fish_config_keys_create_cb(data, config_file, section, option_name, value):
    secure_key = fish_secure_key()
    if secure_key and not value.startswith('+OK '):
        value = blowcrypt_pack(value.encode(), secure_key)
        option_name = blowcrypt_pack(option_name.encode(), secure_key)
...
def fish_config_keys_delete_cb(data, config_file, section, option):
    ...
    if secure_key:
        try:
            option_name, _ = blowcrypt_unpack(option_name, secure_key)
            option_name = option_name.decode()
        except Exception:
            pass
...
def fish_config_init():
    ...
    fish_config_option["mark_position"] = weechat.config_new_option(
        fish_config_file, section_look, "mark_position", "integer",
        "put marker for encrypted INCOMING messages at start or end",
        "off|begin|end", 0, 2, "off", "off", 0, "", "", "", "", "", "")

    fish_config_option["mark_encrypted"] = weechat.config_new_option(
        fish_config_file, section_look, "mark_encrypted", "string",
        "marker for encrypted INCOMING messages", "", 0, 0,
        "*", "*", 0, "", "", "", "", "", "")
    ...
    # secure
    section_secure = weechat.config_new_section(
        fish_config_file, "secure", 0, 0, "", "", "", "", "", "", "", "", "",
        "")
    if not section_secure:
        weechat.config_free(fish_config_file)
        return

    fish_config_option["key"] = weechat.config_new_option(
        fish_config_file, section_secure, "key",
        "string", "key for encrypting blowfish keys", "", 0, 0,
        "", "", 0, "", "", "", "", "", "")
...
def fish_key_get(target: str):
    target = target.lower()
    if secure_key := fish_secure_key():
        target = blowcrypt_pack(target.encode(), secure_key)
    ...
    key = weechat.config_string(option)
    if secure_key:
        key, _ = blowcrypt_unpack(key, secure_key)
        key = key.decode()
...
def blowcrypt_pack(msg, key, cbc=False):
    """."""
    if cbc:
...
def fish_modifier_out_encrypt_cb(data, modifier, server_name, string):
    ...
    key, cbc = key
    text = fish_msg_wo_marker(msg_info['text'])
...
def fish_modifier_input_text(data, modifier, server_name, string):
    ...
    if fish_key_get(target.lower()) is None:
        return string
...
def fish_cmd_blowkey(data, buffer, args):
    ...
    if args in ['', 'list']:
        ...
        if fish_secure_key():
            weechat.prnt(buffer, 'Key already set.')
            return weechat.WEECHAT_RC_ERROR
        newKey = blowcrypt_b64encode(urandom(32))
        weechat.command(
            buffer, f'/set {CONFIG_FILE_NAME}.secure.key ${{sec.data.fish}}')
        weechat.command(buffer, f'/secure set fish {newKey}')
...
def fish_secure_key():
    ...
    return weechat.string_eval_expression(
        weechat.config_string(fish_config_option["key"]), {}, {}, {})
...
def fish_msg_w_marker(msg):
    marker = weechat.config_string(
        fish_config_option["mark_encrypted"]).encode()
